        ...
        for idx, (taxonomy_ids, model_ids, data) in tqdm(enumerate(test_dataloader), total=len(test_dataloader), desc='Test', colour='yellow'):
            ...
            if not config.get('TRAINING_TEST', False):
                if config.dataset.test._base_.get('EXT_MESH', False):
                    ...
                    pred_surface = get_partial_mesh_batch( in_dict['input'], ret_dict['coarse'], k=50)
...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    # training
    base_model.zero_grad()
    for epoch in tqdm(range(start_epoch, config.max_epoch + 1), total=(config.max_epoch+1-start_epoch), colour='green', desc='Epoch'):
        if args.distributed:
            train_sampler.set_epoch(epoch)
        base_model.train()
        ...
        for idx, (taxonomy_ids, model_ids, data) in tqdm(enumerate(train_dataloader), total=len(train_dataloader), desc='Iter', colour='yellow'):
            ...
            if dataset_name == 'PCN':
                ...
                if config.dataset.train._base_.CARS:
                    ...
                    # Subsamples using Furthest Point Sampling. Easier to put it here cause it's epoch-dependent
                    # Higher epoch -> drop more points to make it harder for the model
            ...
            elif dataset_name == 'VC':
                partial = data[0].cuda()
                gt = data[1].cuda()
                if not config.dataset.train.fixed_input:
                    num_pts_per_idx = data[2]['num_pts']

            else:
                raise NotImplementedError(f'Training phase does not support {dataset_name}')

            num_iter += 1
           
            if config.dataset.train.fixed_input:
                ret = base_model(partial)
            else:    
                ret = base_model(partial, num_pts_per_idx)
    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
            ...
            if dataset_name == 'PCN':
                ...
                partial = data[0].cuda()
                gt = data[1].cuda()
                if not config.dataset.val.fixed_input:
                    num_pts_per_idx = data[2]['num_pts']
            else:
            ...
            if val_writer is not None and idx % 200 == 0:
                ...
                val_writer.add_image(f'{img_id}/Input', input_pc, epoch, dataformats='HWC')

                sparse = coarse_points.squeeze().cpu().numpy()
                sparse_img = misc.get_ptcloud_img(sparse)
                val_writer.add_image(f'{img_id}/Coarse', sparse_img, epoch, dataformats='HWC')

                dense = dense_points.squeeze().cpu().numpy()
                dense_img = misc.get_ptcloud_img(dense)
                val_writer.add_image(f'{img_id}/Dense', dense_img, epoch, dataformats='HWC')
                
                gt_ptcloud = gt.squeeze().cpu().numpy()
                gt_ptcloud_img = misc.get_ptcloud_img(gt_ptcloud)
                val_writer.add_image(f'{img_id}/GT', gt_ptcloud_img, epoch, dataformats='HWC')
    ...
    with torch.no_grad():
        for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
            ...
            if dataset_name in ['PCN', 'VC']:
                ...
                if not os.path.exists(target_path):
                    os.mkdir(target_path)
                target_path = os.path.join(target_path, f'{label["ids"]}')
                if not os.path.exists(target_path):
                    os.mkdir(target_path)
                np.save(os.path.join(target_path, 'input.npy'), partial.cpu().numpy())
                np.save(os.path.join(target_path, 'gt.npy'), gt.cpu().numpy())
                np.save(os.path.join(target_path, 'coarse.npy'), coarse_points.cpu().numpy())
                np.save(os.path.join(target_path, 'dense.npy'), dense_points.cpu().numpy())
