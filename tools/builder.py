...
def dataset_builder(args, config):
    ...
    if config._base_.NAME in ['VC','KITTI']:
        if config.others.fixed_input:
            ...
            print('Using variable input model')
    ...
    if args.distributed:
        ...
        sampler = None
        dataloader = torch.utils.data.DataLoader(dataset, batch_size=config.others.bs if shuffle else 16,
                                                shuffle = shuffle, 
                                                drop_last = config.others.subset == 'train',
                                                num_workers = int(args.num_workers),
                                                worker_init_fn=worker_init_fn,
...
def build_opti_sche(base_model, config, steps_per_epoch, epochs):
    ...
    elif sche_config.type == 'OneCycle':
        scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, steps_per_epoch=steps_per_epoch, epochs=epochs, **sche_config.kwargs)
    else:
        raise NotImplementedError()
